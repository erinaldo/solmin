...
#Region "Exportar Excel"
        ...
        For Each dr As DataRow In DtReporte.Rows
            ...
            Else
                For Each drAux As DataRow In DtReporte.Select("ISNULL(TPRVCL,'') = '" & ("" & dr("TPRVCL")) & "'")
                    ...
                    End If
                    nDiaAtraso = nDiaAtraso + IIf(drAux("NRDIA_CA") > 0, drAux("NRDIA_CA"), 0)
    ...
    Private Sub ExportarExcelOC(ByVal DtReporte As DataTable)
        ...
        oDt.Columns("NRDIA_CA").ColumnName = String.Format("DIAS DE ATRAZO " & Chr(10) & "PROMETIDO ({0} - A)", strLetra)
      
        If oDt.Rows.Count > 0 Then
            If oDt.Rows(0).Item("CHECK_148") = 1 Then
                oDt.Columns("NRDIA_EC").ColumnName = String.Format(" DIAS DE ATRAZO " & Chr(10) & " ENTREGA ({0} - {1})", strLetra1, strLetra)
                strLetra = "D"
            Else
                oDt.Columns.Remove("NRDIA_EC")
            End If
            If oDt.Rows(0).Item("CHECK_3") = 1 Then
                oDt.Columns("NRDIA_ED").ColumnName = String.Format(" DIAS DE ATRAZO " & Chr(10) & " RECOJO ({0} - {1})", strLetra1, strLetra)
            Else
        ...
        oDt.Columns("TOBS").ColumnName = "BITACORA"
