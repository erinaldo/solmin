...
Public Class frmAgregarItemsBulto
...
#Region " Métodos "
    ...
    Private Sub btnCancelar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbCancelar.Click
        If mfblnSalirOpcion() Then
            Me.DialogResult = Windows.Forms.DialogResult.Cancel
        End If
    End Sub

    Private Sub btnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbGuardar.Click
...
#End Region
    ...
    Private Sub pMarcarFila(ByVal Indice As Int32, ByVal Status As Boolean)
        ...
        If Status Then
            Me.dgOrdenComprasSeleccionados.Rows(Indice).Cells("QCNREC").Value = CType(dgOrdenComprasSeleccionados.Rows(Indice).DataBoundItem, beOrdenCompra).PNQCNPEN
...
#Region "Metodos"
    ...
    Private Sub CargarDetalleOrdeDeCompraXBulto()
        ...
        If pTipoMaterial = "MATPEL" Then
            ...
            For i As Integer = 0 To dgOrdenComprasSeleccionados.Rows.Count - 1
                ...
                End If
                dgOrdenComprasSeleccionados.Rows(i).Cells("GrillaFechaCad").Value = HelpClass.FechaNum_a_Fecha(dgOrdenComprasSeleccionados.Rows(i).Cells("GrillaFechaCad").Value.ToString())
        ...
        For Each obeOrdeDeCompra As beOrdenCompra In CType(Me.dgOrdenComprasSeleccionados.DataSource, List(Of beOrdenCompra))
            If obeOrdeDeCompra.CHK Then
                ...
                With oBeDetalleBulto
                    ...
                    Date.TryParse(txtFechaFactProveedor.Text, .FechaFacturaProveedor)
                    .PSNGRPRV = txtNroGuiaProveedor.Text
                    .PNFFACPR = Val(txtNroFactProveedor.Text)
                    .PSTIPO = _pEstado
                    ...
                    'If (pTipoMaterial <> "MATPEL") Then
                    '    .PSFCHCAD = 0
                    'ElseIf (dgOrdenComprasSeleccionados.Rows(contador).Cells("GrillaFechaCad").Value.ToString().Trim = "") Then
                    '    .PSFCHCAD = 0
                    'Else
                    '    .PSFCHCAD = Convert.ToDateTime(dgOrdenComprasSeleccionados.Rows(contador).Cells("GrillaFechaCad").Value()).ToString("yyyyMMdd")
                    'End If
                    If dgOrdenComprasSeleccionados.Rows(contador).Cells("GrillaFechaCad").Value Is Nothing OrElse dgOrdenComprasSeleccionados.Rows(contador).Cells("GrillaFechaCad").Value.ToString.Trim = "" Then
                        .PSFCHCAD = 0
                    Else
                        .PSFCHCAD = Convert.ToDateTime(dgOrdenComprasSeleccionados.Rows(contador).Cells("GrillaFechaCad").Value()).ToString("yyyyMMdd")
                    End If
    ...
    Private Function fblnValidar() As Boolean
        ...
        If Me.txtNroGuiaProveedor.Text = "" Then _
...
#End Region
    ...
    Private Sub dgOrdenComprasSeleccionados_CellValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellValidatingEventArgs) Handles dgOrdenComprasSeleccionados.CellValidating
        If CType(dgOrdenComprasSeleccionados.CurrentRow.DataBoundItem, beOrdenCompra).CHK Then
            Select Case dgOrdenComprasSeleccionados.Columns(e.ColumnIndex).Name
                ...
                Case "QCNREC"
                    Dim strMensajeError As String = ""
                    If (Val(dgOrdenComprasSeleccionados.CurrentCell.Value) <= 0) Then
    ...
    Private Sub tstOrdenDeCompra_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles tstOrdenDeCompra.KeyDown
        If e.KeyCode = Keys.Enter Then
            _pobeOrdeDeCompra.PSNORCML = Me.tstOrdenDeCompra.Text.Trim
            CargarDetalleOrdeDeCompraXBulto()
        End If
    End Sub
    ...
    Private Sub dgOrdenComprasSeleccionados_CellValueChanged(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgOrdenComprasSeleccionados.CellValueChanged
        If ValidarCellChange = False Then
            Dim ColName As String = dgOrdenComprasSeleccionados.Columns(e.ColumnIndex).Name.ToString().Trim
    ...
    Private Sub dgOrdenComprasSeleccionados_EditingControlShowing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewEditingControlShowingEventArgs) Handles dgOrdenComprasSeleccionados.EditingControlShowing
        If dgOrdenComprasSeleccionados.Columns(dgOrdenComprasSeleccionados.CurrentCell.ColumnIndex).Name.ToString().Trim = "GrillaCodUn" Then
