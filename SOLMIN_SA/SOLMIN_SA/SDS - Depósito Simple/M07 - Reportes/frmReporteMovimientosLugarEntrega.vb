...
Public Class frmReporteMovimientosLugarEntrega

#Region "Declaracion de variables"
    Private objTemp As TipoDato_ResultaReporte
    Private rptTemp = New rptMovimientoLugarEntrega
...
#Region "Procedimientos y funciones"
    ...
    Private Sub pReporteMovimientoxLugarEntrega()

        rptTemp = New rptMovimientoLugarEntrega
        ...
        dsReportes = oReporteSDS.frptListar_MovMercaderias_x_LugarEntrega(Filtro, strMensaje)
        ...
        rptTemp.Subreports.Item("rptResumenClienteTercero").SetDataSource(dsReportes.Tables(1).Copy)
...
#Region "Eventos de Control"
    ...
    Private Sub frmReporteMovimientosLugarEntrega_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub bgwGifAnimado_DoWork(ByVal sender As Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bgwGifAnimado.DoWork
        pReporteMovimientoxLugarEntrega()
    End Sub
    ...
    Private Sub tsbEnviarCorreo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbEnviarCorreo.Click
        Try
            Call mpEnviarEMail(objTemp, "Movimientos por Lugar de Entrega", "Informe :  Movimientos por Lugar de Entrega")
        Catch ex As Exception
...
#End Region
    ...
    Private Sub tsbExportarExcel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbExportarExcel.Click
        ...
        Try
            ...
            dtTemp.Tables(0).Columns("CDPEPL").ColumnName = "PEDIDO"
            dtTemp.Tables(0).Columns("FCHSPE").ColumnName = "FECHA PEDIDO"
            dtTemp.Tables(0).Columns("DESCLI").ColumnName = "CLIENTE PROPIO O TERCERO"
            dtTemp.Tables(0).Columns("TCMPPL").ColumnName = "PLANTA"
            dtTemp.Tables(0).Columns("NDCFCC").ColumnName = "FACTURA"
            dtTemp.Tables(0).Columns("NRFRPD").ColumnName = "REFERENCIA"
            dtTemp.Tables(0).Columns("NORCML").ColumnName = "O/C"
            dtTemp.Tables(0).Columns("TOBSMD").ColumnName = "OBSERVACIÓN"
            dtTemp.Tables(0).Columns("CDREGP").ColumnName = "ITEM"
            dtTemp.Tables(0).Columns("CMRCLR").ColumnName = "CÓDIGO"
            dtTemp.Tables(0).Columns("DESMER").ColumnName = "DESCRIPCIÓN"
            dtTemp.Tables(0).Columns("FDSPAL").ColumnName = "FECHA DESPACHO"
            dtTemp.Tables(0).Columns("QDSMC").ColumnName = "CANTIDAD"
            dtTemp.Tables(0).Columns("CUNCN6").ColumnName = "UNIDAD"

            dtTemp.Tables(0).Columns.Remove("NORDSR")
            dtTemp.Tables(0).Columns.Remove("TDRCPL")
            dtTemp.Tables(0).Columns.Remove("CCLNT")
            dtTemp.Tables(0).Columns.Remove("CPLNCL")

            dtTemp.Tables(0).TableName = "LUGAR-ENTREGA"

            dtTemp.Tables(1).Columns("DESCLI").ColumnName = "CLIENTE PROPIO O TERCERO"
            dtTemp.Tables(1).Columns("TCMPPL").ColumnName = "PLANTA"
            dtTemp.Tables(1).Columns("QDSMC").ColumnName = "UNIDAD"

            dtTemp.Tables(1).Columns.Remove("TDRCPL")
            dtTemp.Tables(1).Columns.Remove("CCLNT")
            dtTemp.Tables(1).Columns.Remove("CPLNCL")

            dtTemp.Tables(1).TableName = "RESUMEN-POR-PLANTA-ENTREGA"
