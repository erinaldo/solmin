...
Public Class frmReporteRotacionProductoAnual
    ...
    Private Sub frmReporteRotacionProductoMensual_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'me..HabilitarCompaniaActual(RANSA.Utilitario.MainModuleDeploy.IdCompaniaDeploy)
    ...
    Private Sub pGeneraReporte()
        ...
        tabla2.Columns.Add(New DataColumn("IMVTAS", GetType(Decimal)))
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            objDataRow.Item("IMVTAS") = dt.Rows(i)("IMVTAS").ToString.Trim
        ...
        tablafinal.Columns.Add(New DataColumn("IMVTAS", GetType(String)))
        tablafinal.Columns.Add(New DataColumn("STOCK_INI", GetType(String)))
        tablafinal.Columns.Add(New DataColumn("VALOR_INI", GetType(String)))
        For i As Integer = 1 To 12
        ...
        tablafinal.Columns.Add(New DataColumn("VALOR_FIN", GetType(String)))
        ...
        tablafinal.Columns.Add(New DataColumn("TOTAL_ING", GetType(String)))
        tablafinal.Columns.Add(New DataColumn("SUMA_DES", GetType(String)))
        tablafinal.Columns.Add(New DataColumn("TOTAL_FIN", GetType(String)))

        tablafinal.Columns.Add(New DataColumn("PORCENTAJE", GetType(Decimal)))
        tablafinal.Columns.Add(New DataColumn("ROTACION", GetType(String)))
        tablafinal.Columns.Add(New DataColumn("PARTICIPACION", GetType(String)))
        tablafinal.Columns.Add(New DataColumn("ANALISIS", GetType(String)))
        ...
        For Each row As DataRow In tablaauxiliar.Rows
            ...
            Try
                ...
                newrow("IMVTAS") = row("IMVTAS")

                newrow("CMRCLR") = row("CMRCLR")
                newrow("TMRCD2") = row("TMRCD2")
                newrow("STOCK_INI") = row("STOCK_INI")
                If row("STOCK_INI").ToString.Length > 0 Then
                    newrow("VALOR_INI") = row("IMVTAS") * row("STOCK_INI")
                End If
                ...
                If row("STOCK_FIN").ToString.Length > 0 Then
                    newrow("VALOR_FIN") = row("IMVTAS") * row("STOCK_FIN")
                End If
                ...
                If newrow("SUMA_ING").ToString.Length > 0 Then
                    newrow("TOTAL_ING") = row("IMVTAS") * newrow("SUMA_ING")
                End If
                newrow("SUMA_DES") = IIf(sumades = 0, "", sumades)

                If newrow("SUMA_DES").ToString.Length > 0 Then
                    newrow("TOTAL_FIN") = row("IMVTAS") * newrow("SUMA_DES")
                End If
                ...
                Try
                    rotacion = IIf(newrow("SUMA_DES") = "", "0.00", newrow("SUMA_DES")) / IIf(row("STOCK_PROM") = "", "0.00", row("STOCK_PROM"))
                    newrow("ROTACION") = IIf(rotacion > 0, rotacion, "0.00")
                Catch ex As Exception
                    newrow("ROTACION") = "0.00"
                End Try



                newrow("ANALISIS") = IIf(newrow("ROTACION") <= 1, 1, IIf(newrow("ROTACION") < 6, 2, 3))
        ...
        Dim Participacion As Decimal = 0
        For Each dr As DataRow In tablafinal.Rows
            Participacion = Participacion + IIf(dr("SUMA_DES").ToString.Length > 0, dr("SUMA_DES"), "0.00")
        Next
        nCount = 0
        For Each dr As DataRow In tablafinal.Rows
            If dr("SUMA_DES").ToString.Length > 0 Then
                'tablafinal.Rows(nCount).Item("PARTICIPACION") = Math.Round(((dr("SUMA_DES") / Participacion) * 100), 2) & "%"
                tablafinal.Rows(nCount).Item("PARTICIPACION") = dr("SUMA_DES") / Participacion
            Else
                tablafinal.Rows(nCount).Item("PARTICIPACION") = "0.00"
            End If
            If dr("ROTACION") = 0 Then
                dr("ROTACION") = ""
            End If
            If dr("PARTICIPACION") = 0 Then
                dr("PARTICIPACION") = ""
            End If
            ...
            If dr("VALOR_INI") Is DBNull.Value Then
                dr("VALOR_INI") = ""
            Else
                If dr("VALOR_INI") = "0" Then
                    dr("VALOR_INI") = ""
                End If
            End If

            If dr("VALOR_FIN") Is DBNull.Value Then
                dr("VALOR_FIN") = ""
            Else
                If dr("VALOR_FIN") = "0" Then
                    dr("VALOR_INI") = ""
                End If
            End If

            If dr("TOTAL_ING") Is DBNull.Value Then
                dr("TOTAL_ING") = ""
            Else
                If dr("TOTAL_ING") = "0" Then
                    dr("TOTAL_ING") = ""
                End If
            End If

            If dr("TOTAL_FIN") Is DBNull.Value Then
                dr("TOTAL_FIN") = ""
            Else
                If dr("TOTAL_FIN") = "0" Then
                    dr("TOTAL_FIN") = ""
                End If
            End If
    ...
    Private Sub exportarexcel()
        ...
        dtExcel.Columns("IMVTAS").ColumnName = "VALOR UNITARIO S/."

        dtExcel.Columns("STOCK_INI").ColumnName = "STOCK INICIAL \n CANTIDAD"
        dtExcel.Columns("VALOR_INI").ColumnName = "STOCK INICIAL \n VALOR"
        ...
        dtExcel.Columns("VALOR_FIN").ColumnName = "STOCK FINAL \n VALOR"

        dtExcel.Columns("STOCK_PROM").ColumnName = ("STOCK PROMEDIO")

        dtExcel.Columns("SUMA_ING").ColumnName = "TOTAL INGRESO \n CANTIDAD"
        dtExcel.Columns("TOTAL_ING").ColumnName = "TOTAL INGRESO \n VALOR"

        dtExcel.Columns("SUMA_DES").ColumnName = "TOTAL DESPACHO \n CANTIDAD"
        dtExcel.Columns("TOTAL_FIN").ColumnName = "TOTAL DESPACHO \n VALOR"

        dtExcel.Columns("ROTACION").ColumnName = "ROTACION  DE INVENTARIO "
        dtExcel.Columns("PARTICIPACION").ColumnName = "PARTICIPACION DE OPERACION "
        dtExcel.Columns("ANALISIS").ColumnName = "ANALISIS DE ROTACION "
