...
Public Class frmNotaCredito
    ...
    Private Sub frmNotaCredito_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Dim dr() As DataRow
            dr = dtImporteFac.Select("NDCCTC='" & _pLIST_NOPRCN(0).NDCCTC & "' AND CTPODC=1")
            If dr.Length > 0 Then
                'txtImporteFactura.Text = Format(dr(0)("IMPORTE"), "###########0.00000")
            ...
            _dtServiciosTodosOp = oServicioOpeNeg.Lista_Operacion_Servicio_Refactura(oLisServicio)
    ...
    Private Sub RecalcularMonto()
        ...
        For Fila As Int64 = 0 To _dtServiciosTodosOp.Rows.Count - 1
            ...
            _dtServiciosTodosOp.Rows(Fila)("MONTO") = QATNAN * IVLSRV
        Next
    ...
    Private Sub Generar()
        ...
        _pLIST_NOPRCN = New List(Of Ransa.Controls.ServicioOperacion.Servicio_BE)
        dtgOperaciones.EndEdit()
