...
#Region " Funciones y Procedimientos "
    Private Sub pCargarInformacion()
        ...
        dgCliente.DataSource = oCliente.Listar(oQry_Cliente_L01, sAccesoPorUsuario, intNroTotalPaginas, intTipoCliente)
        ...
        Else
            If dgCliente.RowCount > 0 Then
                ...
                oInf_Cliente.pTCMPCL_DescripcionCliente = ("" & dgCliente.CurrentRow.Cells("M_TCMPCL").Value).ToString.Trim
                ...
                '<[AHM]>
                oInf_Cliente.pCRGVTA_CodigoRegionVenta = dgCliente.CurrentRow.Cells("M_CRGVTA").Value
                oInf_Cliente.pTCRVTA_DescripcionRegionVenta = dgCliente.CurrentRow.Cells("M_TCRVTA").Value
                oInf_Cliente.pCDSCSP_CodigoSector = dgCliente.CurrentRow.Cells("M_CDSCSP").Value
                oInf_Cliente.pCDDRSP_CodClienteSAP = dgCliente.CurrentRow.Cells("M_CDDRSP").Value
                '</[AHM]>
...
#Region " Eventos "
    ...
    Private Sub dgCliente_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles dgCliente.KeyDown
        If e.KeyCode = Keys.Enter Then If blnSalirDoubleClick Then RaiseEvent SalirDobleClick(oInf_Cliente)
    End Sub
...
#Region " Métodos "
    ...
    Public Sub LoadDataSource(ByVal objFiltro As TD_Qry_Cliente_L01, ByVal DataSource As DataTable, ByVal iTPag As Integer, Optional ByVal iTClient As Integer = 0)
        ...
        oQry_Cliente_L01.pREGPAG_NroRegPorPagina = objFiltro.pREGPAG_NroRegPorPagina
        oQry_Cliente_L01.pCCMPN_CodigoCompania = objFiltro.pCCMPN_CodigoCompania
        oQry_Cliente_L01.pVisualizarNegocio = objFiltro.pVisualizarNegocio

        dgCliente.Columns("M_TCRVTA").Visible = oQry_Cliente_L01.pVisualizarNegocio
        ...
        Else
            If dgCliente.RowCount > 0 Then
                ...
                oInf_Cliente.pNLBTEL_NroDocIdentidad = dgCliente.CurrentRow.Cells("M_NLBTEL").Value
                '<[AHM]>
                oInf_Cliente.pCRGVTA_CodigoRegionVenta = ("" & dgCliente.CurrentRow.Cells("M_CRGVTA").Value).ToString().Trim()
                oInf_Cliente.pTCRVTA_DescripcionRegionVenta = ("" & dgCliente.CurrentRow.Cells("M_TCRVTA").Value).ToString().Trim()
                oInf_Cliente.pCDSCSP_CodigoSector = ("" & dgCliente.CurrentRow.Cells("M_CDSCSP").Value).ToString().Trim()
                oInf_Cliente.pCDDRSP_CodClienteSAP = ("" & dgCliente.CurrentRow.Cells("M_CDDRSP").Value).ToString().Trim()
                '</[AHM]>
                intFilaActual = 0
                RaiseEvent DataLoadCompleted(oInf_Cliente)
            Else
