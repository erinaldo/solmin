...
Public Class HelpClass_NPOI_AD
    ...
    Private Const pFilaSupCabecera As String = "FILA_SUP_CABECERA"
    ...
    Public Shared Sub ExportExcelGeneralRelease(ByVal odtDataSource As DataTable, ByVal Titulo As String, ByVal ListColNameNFilas As String, Optional ByVal filtro As SortedList = Nothing)
        ...
        Else
            ...
            For Each items As DictionaryEntry In filtro
                ...
                objWorkSheet.AddMergedRegion(New NPOI.SS.Util.Region(rowActual + x - 4, 1, rowActual + x - 4, 4))
            Next
    ...
    Public Sub ExportExcelGeneralMultipleValorizacion(ByVal odtListDataSource As List(Of DataTable), ByVal ListTitulo As List(Of String), ByVal ListColNameNFilas As List(Of String), Optional ByVal ListFiltro As List(Of SortedList) = Nothing, Optional ByVal ListNameDelFilDuplic As List(Of String) = Nothing, Optional MismaMonedaAdmin As Boolean = False)
        ...
        Dim styleNormal As NPOI.SS.UserModel.CellStyle = objWorkBook.CreateCellStyle()
        Dim styleNumber As NPOI.SS.UserModel.CellStyle = objWorkBook.CreateCellStyle()
        Dim styleNumberResumen As NPOI.SS.UserModel.CellStyle = objWorkBook.CreateCellStyle()
        Dim styleMonto As NPOI.SS.UserModel.CellStyle = objWorkBook.CreateCellStyle()
        Dim styleFecha As NPOI.SS.UserModel.CellStyle = objWorkBook.CreateCellStyle()
        Dim styleTexto As NPOI.SS.UserModel.CellStyle = objWorkBook.CreateCellStyle()
        Dim patriarch As HSSFPatriarch = DirectCast(objWorkSheet.CreateDrawingPatriarch(), HSSFPatriarch)
        ...
        Dim oFontSumaResumen As NPOI.SS.UserModel.Font = objWorkBook.CreateFont
        oFontSumaResumen.Color = NPOI.HSSF.Util.HSSFColor.BLACK.index
        oFontSumaResumen.Boldweight = NPOI.SS.UserModel.FontBoldWeight.BOLD
        oFontSumaResumen.FontHeight = fontSizeCelda
        ...
        Estilos_Excel_NPOI(styleNumberResumen, TipoEstilo.Numero)
        styleNumberResumen.SetFont(oFontSumaResumen)
        ...
        For FilaTable As Integer = 0 To odtListDataSource.Count - 1
            ...
            Dim oHasFormatoFila As New Hashtable
            Dim ValCelda As String = ""
            Dim oHasColumnaPosicion As New Hashtable
            Dim FilaInicioListado As Int32 = 0
            'Dim FilaInicioListadoAsig As Boolean = False

            '  'Administración
            '++++++++++++++++++++++++++++++++++
            Dim NombreCol As String = ""
            If odtDataSource.TableName = "ADMINISTRACION" Then
                For i As Integer = 0 To odtDataSource.Rows.Count - 1

                    If odtDataSource.Rows(i)("NSECFC") = "X" Then
                        For j As Integer = 0 To odtDataSource.Columns.Count - 1
                            y = IndiceColum
                            NombreCol = odtDataSource.Columns(j).ColumnName
                            ValorCelda = ("" & odtDataSource.Rows(i)(NombreCol))

                            If NombreCol.StartsWith("SERV_") Then
            ...
            For j As Integer = 0 To odtDataSource.Columns.Count - 1
                ...
                oHasColumnaPosicion(odtDataSource.Columns(j).ColumnName) = y
            ...
            FilaInicioListado = indice - 1 + rowActual
            For i As Integer = 0 To odtDataSource.Rows.Count - 1
            ...
            '+++++++++++++PARA AMBOS++++++++++++++++++++++++++++++
            If oHasColumnaPosicion.Contains("IMPVLR_SOL") And oHasColumnaPosicion.Contains("IMPVLR_DOL") Then
                Dim I_LetraVLR_SOL As Int32 = oHasColumnaPosicion("IMPVLR_SOL")
                Dim I_LetraVLR_DOL As Int32 = oHasColumnaPosicion("IMPVLR_DOL")
                Dim FormulaSumaVLR_SOL As String = "SUM(" & HelpClass.LetraNumero(I_LetraVLR_SOL + 1) & (FilaInicioListado).ToString & ":" & HelpClass.LetraNumero(I_LetraVLR_SOL + 1) & (FilaInicioListado + odtDataSource.Rows.Count).ToString & ")"
                Dim FormulaSumaVLR_DOL As String = "SUM(" & HelpClass.LetraNumero(I_LetraVLR_DOL + 1) & (FilaInicioListado).ToString & ":" & HelpClass.LetraNumero(I_LetraVLR_DOL + 1) & (FilaInicioListado + odtDataSource.Rows.Count).ToString & ")"
                objWorkSheet.GetRow(indice - 1 + rowActual).CreateCell(I_LetraVLR_SOL).CellStyle = styleNumberResumen
                objWorkSheet.GetRow(indice - 1 + rowActual).GetCell(I_LetraVLR_SOL).CellFormula = (FormulaSumaVLR_SOL)
                objWorkSheet.GetRow(indice - 1 + rowActual).CreateCell(I_LetraVLR_DOL).CellStyle = styleNumberResumen
                objWorkSheet.GetRow(indice - 1 + rowActual).GetCell(I_LetraVLR_DOL).CellFormula = (FormulaSumaVLR_DOL)
            End If
            '++++++++++++++++++++++++++++++++++++++++++++++++++++++

            ' PARA OPERACIONES DE ADMINISTRACION
            '++++++++++++++++++++++++++++++++++++++++++++++++++++++
            If MismaMonedaAdmin = True And oHasColumnaPosicion.Contains("IMPOVLR_CALC") Then
                Dim I_LetraVLR_RECALCULO As Int32 = oHasColumnaPosicion("IMPOVLR_CALC")
                Dim FormulaSumaRecalculo As String = "SUM(" & HelpClass.LetraNumero(I_LetraVLR_RECALCULO + 1) & (FilaInicioListado).ToString & ":" & HelpClass.LetraNumero(I_LetraVLR_RECALCULO + 1) & (FilaInicioListado + odtDataSource.Rows.Count).ToString & ")"
                objWorkSheet.GetRow(indice - 1 + rowActual).CreateCell(I_LetraVLR_RECALCULO).CellStyle = styleNumberResumen
                objWorkSheet.GetRow(indice - 1 + rowActual).GetCell(I_LetraVLR_RECALCULO).CellFormula = (FormulaSumaRecalculo)
            End If
            '++++++++++++++++++++++++++++++++++++++++++++++++++++++
