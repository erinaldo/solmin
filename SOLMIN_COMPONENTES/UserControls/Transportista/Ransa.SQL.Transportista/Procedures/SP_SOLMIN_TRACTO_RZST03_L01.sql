DROP PROCEDURE SP_SOLMIN_TRACTO_RZST03_L01
GO
CREATE PROCEDURE SP_SOLMIN_TRACTO_RZST03_L01(
...
IF IN_TMRCTR <> '' THEN 
	IF POSSTR ( IN_TMRCTR , ',' ) <> 0 THEN 
		...
		SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND TMRCTR IN  ( ''' || IN_TMRCTR || ''') ' ; 
	ELSE 
		IF POSSTR ( IN_TMRCTR , '*' ) <> 0 THEN 
			SET IN_TMRCTR = REPLACE ( IN_TMRCTR , '*' , '%' ) ; 
			SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND TMRCTR LIKE ''' || IN_TMRCTR || ''' ' ; 
		ELSE 
			SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND TMRCTR = ''' || IN_TMRCTR || ''' ' ; 
		END IF ; 
	END IF ; 
END IF ; 
...
IF IN_NRGMT1 <> '' THEN 
	IF POSSTR ( IN_NRGMT1 , ',' ) <> 0 THEN 
		...
		SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND NRGMT1 IN  ( ''' || IN_NRGMT1 || ''') ' ; 
	ELSE 
		IF POSSTR ( IN_NRGMT1 , '*' ) <> 0 THEN 
			SET IN_NRGMT1 = REPLACE ( IN_NRGMT1 , '*' , '%' ) ; 
			SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND Trim(NRGMT1) LIKE ''' || IN_NRGMT1 || ''' ' ; 
		ELSE 
			SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND NRGMT1 = ''' || IN_NRGMT1 || ''' ' ; 
		END IF ; 
	END IF ; 
END IF ; 

SET STRSQL = ' SELECT Count(NPLCUN) FROM RZST03 WHERE CCMPN = ? AND SESTRG <> ''*'' ' || STRSQL_FILTRO;	 
...
SET STRSQL = ' SELECT NPLCUN FROM RZST03 WHERE CCMPN = ? AND SESTRG <> ''*'' and NPLCUN >= ?  ' || STRSQL_FILTRO || 
...
WHILE WK_CNTREG < ( IN_NROPAG - 1 ) DO 
	OPEN CU_01 USING IN_CCMPN, WK_NPLCUN ; 
...
SET STRSQL = ' SELECT NPLCUN, TMRCTR, NRGMT1 FROM RZST03 WHERE CCMPN = ? AND SESTRG <> ''*'' and NPLCUN >= ? ' || STRSQL_FILTRO || 
...
OPEN CU_02 USING IN_CCMPN, WK_NPLCUN;

END
GO
GRANT ALL PRIVILEGES ON PROCEDURE SP_SOLMIN_TRACTO_RZST03_L01 TO PUBLIC