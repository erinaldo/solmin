DROP PROCEDURE SP_SOLMIN_ACOPLADO_RZST05_L01
GO
CREATE PROCEDURE SP_SOLMIN_ACOPLADO_RZST05_L01(
...
IF IN_TMRCVH <> '' THEN 
	IF POSSTR ( IN_TMRCVH , ',' ) <> 0 THEN 
		...
		SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND TMRCVH IN  ( ''' || IN_TMRCVH || ''') ' ; 
	ELSE 
		IF POSSTR ( IN_TMRCVH , '*' ) <> 0 THEN 
			SET IN_TMRCVH = REPLACE ( IN_TMRCVH , '*' , '%' ) ; 
			SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND TMRCVH LIKE ''' || IN_TMRCVH || ''' ' ; 
		ELSE 
			SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND TMRCVH = ''' || IN_TMRCVH || ''' ' ; 
		END IF ; 
	END IF ; 
END IF ; 
...
IF IN_TDEACP <> '' THEN 
	IF POSSTR ( IN_TDEACP , ',' ) <> 0 THEN 
		...
		SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND EXISTS( SELECT TDEACP FROM RZST04 WHERE CTIACP = RZST05.CTIACP AND TDEACP IN  ( ''' || IN_TDEACP || ''') ) ' ; 
	ELSE 
		IF POSSTR ( IN_TDEACP , '*' ) <> 0 THEN 
			SET IN_TDEACP = REPLACE ( IN_TDEACP , '*' , '%' ) ; 
			SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND EXISTS( SELECT TDEACP FROM RZST04 WHERE CTIACP = RZST05.CTIACP AND Trim(TDEACP) LIKE ''' || IN_TDEACP || ''' ) ' ; 
		ELSE 
			SET STRSQL_FILTRO = STRSQL_FILTRO || ' AND EXISTS( SELECT TDEACP FROM RZST04 WHERE CTIACP = RZST05.CTIACP AND TDEACP = ''' || IN_TDEACP || ''' ) ' ; 
		END IF ; 
	END IF ; 
END IF ; 

SET STRSQL = ' SELECT Count(NPLCAC) FROM RZST05 WHERE CCMPN = ? AND SESTRG <> ''*'' ' || STRSQL_FILTRO;	 
...
SET STRSQL = ' SELECT NPLCAC FROM RZST05 WHERE CCMPN = ? AND SESTRG <> ''*'' AND NPLCAC >= ?  ' || STRSQL_FILTRO || 
...
WHILE WK_CNTREG < ( IN_NROPAG - 1 ) DO 
	OPEN CU_01 USING IN_CCMPN, WK_NPLCAC;
...
SET STRSQL = ' SELECT NPLCAC, TMRCVH, ( SELECT Trim( TDEACP ) FROM RZST04 WHERE CTIACP = RZST05.CTIACP ) TDEACP ' || 
			 ' FROM RZST05 WHERE CCMPN = ? AND SESTRG <> ''*'' and NPLCAC >= ? ' || STRSQL_FILTRO || 
...
OPEN CU_02 USING IN_CCMPN, WK_NPLCAC ; 

END
GO
GRANT ALL PRIVILEGES ON PROCEDURE SP_SOLMIN_ACOPLADO_RZST05_L01 TO PUBLIC 