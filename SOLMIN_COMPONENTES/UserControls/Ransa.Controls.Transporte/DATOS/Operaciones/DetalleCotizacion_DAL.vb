...
Public Class DetalleCotizacion_DAL
  ...
  End Sub
        ...
        Try
            ...
            For Each objDataRow As DataRow In objDataTable.Rows
                ...
                Select Case objEntidad.SESTOS
                    ...
                    Case ""
                        If objDataRow("NORSRT") > 0 Then
                            objEntidad.ESTADOOS = "POR APROBAR"
                        End If
                End Select
    ...
    Public Function ListaCotizacionPorAprobar(ByVal lobjEntidad As Hashtable) As List(Of Detalle_Cotizacion)
        ...
        Dim objServicioMercaderia As New ServicioMercaderia_DAL
        Dim objParam As New Parameter
        Dim objDataTable As New DataTable
        Dim objEntidad As New Detalle_Cotizacion
        ...
        objParam.Add("PNNORSRT", lobjEntidad("PNNORSRT"))
        objParam.Add("PNFCHCRT_INI", CDec(lobjEntidad("PNFCHCRT_INI")))
        objParam.Add("PNFCHCRT_FIN", CDec(lobjEntidad("PNFCHCRT_FIN")))

        objSql.DefaultSchema = Autenticacion_DAL.GetLibrary(lobjEntidad("PSCCMPN"))

        objDataTable = objSql.ExecuteDataTable("SP_SOLMIN_ST_LISTA_COTIZACION_POR_APROBAR", objParam)

        For Each objDataRow As DataRow In objDataTable.Rows
            objEntidad = New Detalle_Cotizacion
            objEntidad.CCMPN = objDataRow("CCMPN").ToString.Trim
            objEntidad.NCRRCT = objDataRow("NCRRCT")
            ...
            objEntidad.CCNCST = objDataRow("CCNCST").ToString().Trim()
            objEntidad.CCNCS1 = objDataRow("CCNCS1").ToString().Trim()
            'objEntidad.MARGEN = objServicioMercaderia.ListaMargenPemitidoxNegocio(objEntidad.CCMPN, ("" & objDataRow("CRGVTA")).ToString.Trim)
            objEntidad.MARGEN = objDataRow("MARGEN_MIN")
