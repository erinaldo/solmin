...
Public Class frmVistaReFactura
  ...
  End Sub
    ...
    Private Sub Datos_Generales_ReFactura_Notas()
        ...
        strCliente = strCliente & vbCrLf & "Tip.Doc.Origen:  " & TipoDocReferencia & " Num.Doc.Origen:  " & lstrConsistenciaOperacion(0).Factura & " Fec.Doc.Origen:  " & IIf(TipoDocumento = 3, HelpClassST.CFecha_10_10(lstrConsistenciaOperacion(0).Fecha), HelpClassST.CFecha_10_10(lstrConsistenciaOperacion(0).Fecha))
    ...
    Private Sub Generar_ReFactura_Notas()
        ...
        Else
            ...
            MessageBox.Show("No existen Facturas para crear", "Aviso", MessageBoxButtons.OK)
        End If
    End Sub
    ...
    For lint_Contador As Integer = 0 To intCant - 1
      ...
      Select Case Me.Tag
        Case 0
          ...
          octrlFact = New ctrlReFactura(oDtDetFactura, lint_Contador + 1, strMoneda, lhashIGV(lint_Contador + 1).ToString.Trim, 0, oFacturaNeg.oTipoDocumento)
          ...
          Me.tabFactura.TabPages(lint_Contador).Controls.Add(octrlFact)
                Case 1
                    ...
                    octrlFact = New ctrlReFactura(oDtDetFacturaDetallada, lint_Contador + 1, strMoneda, lhashIGV(lint_Contador + 1).ToString.Trim, 1, oFacturaNeg.oTipoDocumento)
                    octrlFact.NumFactura = "N°          "
                    octrlFact.Referencia1 = "RUTA: " & oFacturaNeg.Obtener_Referencia_Ruta(ListOperaciones, strCodCom)
                    Me.tabFactura.TabPages(lint_Contador).Controls.Add(octrlFact)
  ...
  Private Sub Generar_Detalles(ByVal pintCant As Integer, ByVal lboolEstado As Integer)
    ...
    Private Sub Grabar_ReFactura(ByVal pintFact As Integer)
        ...
        Try
            ...
            oDtCabFactura.Rows(0).Item("FDCMOR") = HelpClassST.CFecha_de_10_a_8(lstrConsistenciaOperacion(0).Fecha)
            'Next
        ...
        Catch ex As Exception
            ...
            strNumFact = 0
            'HelpClassST.MsgBox("Error al grabar la " & Me.lblTipoDocumento.Text & vbCrLf & ex.Message)
            MessageBox.Show("Error al grabar la " & Me.lblTipoDocumento.Text & vbCrLf & ex.Message, "Aviso", MessageBoxButtons.OK)
        End Try
        If strNumFact <> 0 Then
            Cursor = Cursors.Default
            'HelpClassST.MsgBox("Se grabó correctamente la " & Me.lblTipoDocumento.Text & " N° " & strNumFact)
            MessageBox.Show("Se grabó correctamente la " & Me.lblTipoDocumento.Text & " N° " & strNumFact, "Aviso", MessageBoxButtons.OK)
            Cursor = Cursors.WaitCursor
  ...
  Private Sub Enviar_Factura_SAP(ByVal pstrCompania As String, ByVal pstrTipoDoc As String, ByVal pstrNumFact As String)
    ...
    Catch ex As Exception
            ...
            MessageBox.Show("Error al enviar documento al SAP " & ex.Message, "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning)
    End Try
  End Sub
  ...
  Private Function Registrar_Orden_Interna_Factura(ByVal pstrCompania As String, ByVal pstrTipoDoc As Int32, ByVal pstrNumFact As Long, ByVal pintFecFac As Int64) As Int32
    ...
    Try
      ...
      If oFacturaNeg.Registrar_Orden_Interna_Factura(oFiltro) = 0 Then
                'HelpClassST.MsgBox("Error al enviar Orden Interna", MessageBoxIcon.Information)
                MessageBox.Show("Error al enviar Orden Interna", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        oValor = 0
      Else
        oValor = 1
      End If
