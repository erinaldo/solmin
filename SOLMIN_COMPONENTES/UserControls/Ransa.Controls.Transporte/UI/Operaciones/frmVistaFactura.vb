...
Public Class frmVistaFactura
  ...
  Private strOrgVenta As String = ""
    Private oFacturaNeg As New Operaciones.Factura_Transporte_BLL
  ...
  Private Sub frmVistaFactura_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    If oDt.Rows.Count > 0 Then
      ...
      oFiltro.Add("PNCCLNT", dblCliente)
            oFiltro.Add("PSCUSCRT", _pUsuario) 'USUARIO)
      oFacturaNeg.Llenar_Documentos(oFiltro)
      Me.Llenar_Combos()
      txtDivision.Text = strDivision
    Else
            'HelpClassST.MsgBox("No se puede generar la factura por no existir Tipo de cambio para el día " & Format(Now, "dd/MM/yyyy"))
            MessageBox.Show("No se puede generar la factura por no existir Tipo de cambio para el día " & Format(Now, "dd/MM/yyyy"), "Aviso", MessageBoxButtons.OK)
      Me.Close()
    End If
  ...
  Private Sub btnGenerar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGenerar.Click
    ...
    Select Case strEstado
      Case ""
                'HelpClassST.MsgBox("ERROR : Ocurrio un Problema de Conexión", MessageBoxIcon.Information)
                MessageBox.Show("ERROR : Ocurrio un Problema de Conexión", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Me.btnImprimirfactura.Enabled = ComponentFactory.Krypton.Toolkit.ButtonEnabled.False
      Case "B", "C", "D"
                'HelpClassST.MsgBox("ADVERTENCIA : " & "Cliente " & strResultado, MessageBoxIcon.Information)
                MessageBox.Show("ADVERTENCIA : " & "Cliente " & strResultado, "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Me.btnImprimirfactura.Enabled = ComponentFactory.Krypton.Toolkit.ButtonEnabled.False
    End Select
    Me.Cursor = Cursors.Default
  End Sub
  ...
  Private Sub Generar_Factura()
        ...
        ElseIf intCant = -1 Then
            Me.btnCancelarImpr.Visible = False
            Me.btnImprimirfactura.Visible = False
            'HelpClassST.MsgBox("No se Registró el Peso Volumen de las Operaciones : " & strOperacionSnPeso.Trim)
            MessageBox.Show("No se Registró el Peso Volumen de las Operaciones : " & strOperacionSnPeso.Trim, "Aviso", MessageBoxButtons.OK)
        Else
            Me.btnCancelarImpr.Visible = False
            Me.btnImprimirfactura.Visible = False
            'HelpClassST.MsgBox("No existen Facturas para crear")
            MessageBox.Show("No existen Facturas para crear", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
  End Sub
    Private Sub Mostrar_Factura_Lote()
        ...
        For lint_row As Integer = 0 To dt_LoteFactura.Rows.Count - 1
            ...
            For i As Integer = 0 To drLoteSelect.Length - 1
                ...
                octrlFact = New ctrlFactura(oDtDetFactura, dr("NDCCTC").ToString, strMoneda, lhashIGV(Integer.Parse(dr("NDCCTC").ToString)).ToString.Trim, 0, oFacturaNeg.oTipoDocumento)
    ...
    Private Sub Mostrar_Factura(ByVal intCant As Integer)
        ...
        For lint_Contador As Integer = 0 To intCant - 1
            ...
            Select Case Me.Tag
                Case 0
                    octrlFact = New ctrlFactura(oDtDetFactura, lint_Contador + 1, strMoneda, lhashIGV(lint_Contador + 1).ToString.Trim, 0, oFacturaNeg.oTipoDocumento)
                    octrlFact.NumFactura = "N°          "
                ...
                Case 1
                    ...
                    octrlFact = New ctrlFactura(oDtDetFacturaDetallada, lint_Contador + 1, strMoneda, lhashIGV(lint_Contador + 1).ToString.Trim, 1, oFacturaNeg.oTipoDocumento)
                    octrlFact.NumFactura = "N°          "
                    octrlFact.Referencia1 = "RUTA: " & oFacturaNeg.Obtener_Referencia_Ruta(lstrConsistenciaOperacion, strCodCom)
                    Me.tabFactura.TabPages(lint_Contador).Controls.Add(octrlFact)
  ...
  End Sub
    ...
    Private Sub Grabar_Factura(ByVal pintFact As Integer)
        ...
        Catch ex As Exception
            ...
            strNumFact = 0
            'HelpClassST.MsgBox("Error al grabar la factura " & vbCrLf & ex.Message)
            MessageBox.Show("Error al grabar la factura " & vbCrLf & ex.Message, "Aviso", MessageBoxButtons.OK)
        End Try
        If strNumFact <> 0 Then
            Cursor = Cursors.Default
            'HelpClassST.MsgBox("Se grabó correctamente la Factura N° " & strNumFact)
            MessageBox.Show("Se grabó correctamente la Factura N° " & strNumFact, "Aviso", MessageBoxButtons.OK)
            Cursor = Cursors.WaitCursor
            ...
            If MsgBox("Desea Imprimir Sustento?", MsgBoxStyle.YesNo, "Información") = MsgBoxResult.Yes Then
                ...
                Try
                    ...
                    objDtSustento = HelpClassST.GetDataSetNative(obj_Logica.Listar_Sustento_Factura(objParametroSustento))
                    objDtSustento.TableName = "RZCT01"

                    If objDtSustento.Rows.Count = 0 Then
                        'HelpClassST.MsgBox("No tiene registros", MessageBoxIcon.Information)
                    ...
                    Select Case oDtCabFactura.Rows(0).Item("CTPODC")
                        Case 1
                            ...
                            CType(objetoRepFact.ReportDefinition.ReportObjects("lblUsuario"), CrystalDecisions.CrystalReports.Engine.TextObject).Text = _pUsuario 'MainModule.USUARIO
                        ...
                        Case 7
                            ...
                            CType(objetoRepBole.ReportDefinition.ReportObjects("lblUsuario"), CrystalDecisions.CrystalReports.Engine.TextObject).Text = _pUsuario 'MainModule.USUARIO
        ...
        If strNumFact <> 0 Then
            ...
            If MsgBox("Desea Imprimir Sustento?", MsgBoxStyle.YesNo, "Información") = MsgBoxResult.Yes Then
                ...
                Try
                    ...
                    objDtSustento = HelpClassST.GetDataSetNative(oFacturaNeg.Listar_Sustento_Factura_Lote(objParametroSustento, drSelect))
                    objDtSustento.TableName = "RZCT01"
  ...
  Private Sub Cerrar_Orden_Interna(ByVal pstrCompania As String, ByVal pstrTipoDoc As Int32, ByVal pstrNumFact As Long)
    ...
    Try
      ...
      If oFacturaNeg.Cerrar_Orden_Interna_Factura(oFiltro) = 0 Then
                'HelpClassST.MsgBox("Error al cerrar Orden / Interna Factura ", MessageBoxIcon.Information)
                MessageBox.Show("Error al cerrar Orden / Interna Factura ", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information)
      End If
    Catch : End Try
  ...
  Private Sub Enviar_Factura_SAP(ByVal pstrCompania As String, ByVal pstrTipoDoc As String, ByVal pstrNumFact As String)
    ...
    Try
      ...
      If oDt.Rows.Count > 0 Then
        ...
        Cursor = Cursors.Default
                'HelpClassST.MsgBox("Se envió correctamente al SAP con N° de Documento SAP " & vbCrLf & strNumSAP)
                MessageBox.Show("Se envió correctamente al SAP con N° de Documento SAP " & vbCrLf & strNumSAP, "Aviso", MessageBoxButtons.OK)
      End If
    Catch ex As Exception
            'HelpClassST.MsgBox("Error al enviar documento al SAP " & ex.Message)
  ...
  Private Function Registrar_Orden_Interna_Factura(ByVal pstrCompania As String, ByVal pstrTipoDoc As Int32, ByVal pstrNumFact As Long, ByVal pintFecFac As Int64) As Int32
    ...
    Try
      ...
      If oFacturaNeg.Registrar_Orden_Interna_Factura(oFiltro) = 0 Then
                'HelpClassST.MsgBox("Error al enviar Orden / Interna Factura ", MessageBoxIcon.Information)
                MessageBox.Show("Error al enviar Orden / Interna Factura ", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information)
        oValor = 0
      Else
        oValor = 1
      End If
    ...
    Private Function Registrar_Orden_Interna_Factura_Lote(ByVal pintFact As Integer, ByVal pstrCompania As String, ByVal pstrTipoDoc As Int32, ByVal pstrNumFact As Long, ByVal pintFecFac As Int64) As Int32
        ...
        Try
            ...
            If oFacturaNeg.Registrar_Orden_Interna_Factura_Lote(oFiltro) = 0 Then
                'HelpClassST.MsgBox("Error al enviar Orden / Interna Factura ", MessageBoxIcon.Information)
                MessageBox.Show("Error al enviar Orden / Interna Factura ", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                oValor = 0
            Else
                oValor = 1
            End If
  ...
  Private Sub btnImprimirfactura_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImprimirfactura.Click
        ...
        If strEstado.Trim.Length > 0 Then
            ...
            'HelpClassST.MsgBox("Se excedió Nro Máximo de Item la Factura N° ", MessageBoxIcon.Warning)
            MessageBox.Show("Se excedió Nro Máximo de Item la Factura N° ", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        Else
  ...
  Public Sub Imprime_Factura(ByVal pdblNumFacImp As Int64, ByVal pstrCompania As String, ByVal pstrTipoDoc As String)
    Try
      ...
      strTipoDocFacImp = ""
    Catch ex As Exception
            'HelpClassST.MsgBox("Error al imprimir la factura N° " & pdblNumFacImp & vbCrLf & ex.Message)
            MessageBox.Show("Error al imprimir la factura N° " & pdblNumFacImp & vbCrLf & ex.Message, "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Error)
    End Try
  End Sub
