...
#Region "Eventos Sustento Factura"
  ...
  Private Sub frmOptRepReparto_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Else
            Dim obj_Entidad_TipoDocumento As New mantenimientos.TipoDocumento
            Dim obj_Logica_TipoDocumento As New mantenimientos.TipoDocumento_BLL
      With Me.cboTipoDocumento
                .DataSource = HelpClassST.GetDataSetNative(obj_Logica_TipoDocumento.Listar_Tipo_Documento(obj_Entidad_TipoDocumento))
        .ValueMember = "CTPODC"
  ...
  Private Sub btnProcesarConsulta_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnProcesarConsulta.Click
    Try
      Select Case _lintEstado
        Case 0
                    Dim obj_Logica As New Operaciones.Repartos_BLL
  ...
  Private Sub txtNroFactura_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtNroFactura.KeyPress
    If e.KeyChar = "." Then
      ...
      Exit Sub
    End If
        If HelpClassST.SoloNumeros(CShort(Asc(e.KeyChar))) = 0 Then e.Handled = True
  End Sub
  ...
  Private Sub btnImprimirSustento_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImprimirSustento.Click
    ...
    Try
      ...
      objParametro.Add("PSCDVSN", _CDVSN)
            objDt = HelpClassST.GetDataSetNative(obj_Logica.Listar_Sustento_Factura(objParametro))
      ...
      Select Case Me.cboTipoDocumento.SelectedValue
        Case 1
          objetoRepFact.SetDataSource(objDs)
                    CType(objetoRepFact.ReportDefinition.ReportObjects("lblUsuario"), TextObject).Text = _pUsuario 'MainModule.USUARIO
          ...
          CType(objetoRepFact.ReportDefinition.ReportObjects("lblFactura"), TextObject).Text = "FACTURA N° " & Me.txtNroFactura.Text.Trim
          objPrintForm.ShowForm(objetoRepFact, Me)

        Case 7

          objetoRepBole.SetDataSource(objDs)
                    CType(objetoRepBole.ReportDefinition.ReportObjects("lblUsuario"), TextObject).Text = _pUsuario 'MainModule.USUARIO
          ...
          CType(objetoRepBole.ReportDefinition.ReportObjects("lblFactura"), TextObject).Text = "BOLETA DE VENTA N° " & Me.txtNroFactura.Text.Trim
...
#Region "Eventos Reimpresion de Consistencia por Operacion"
  Private Sub ImprimirConsistencia()
    ...
    Try
      ...
      objParametro.Add("PSCCMPN", _CCMPN)
      objParametro.Add("PSCDVSN", _CDVSN)
            objParametro.Add("PNFECHA", HelpClassST.CDate_a_Numero8Digitos(_Fecha)) 'Me.dtpFechaConsistencia.Value
            objDt = HelpClassST.GetDataSetNative(obj_Logica.Listar_Consistencia_Factura_x_Orden_Servicio(objParametro))
      objDt.TableName = "RZCT01"
      ...
      objetoRep.SetDataSource(objDs)
            ...
            CType(objetoRep.ReportDefinition.ReportObjects("lblUsuario"), TextObject).Text = _pUsuario 'MainModule.USUARIO
    ...
    Private Sub ImprimirPreLiquidacion()
        ...
        Select Case oDtb.Rows(0).Item(0)
            Case 0
                ...
                HelpClassST.CrystalReportTextObject(objetoRep, "lblCompania", _CCMPN_D)
                HelpClassST.CrystalReportTextObject(objetoRep, "lblDivision", _CDVSN_D)
                HelpClassST.CrystalReportTextObject(objetoRep, "lblPlanta", _CPLNDV_D)
                HelpClassST.CrystalReportTextObject(objetoRep, "lblNroPreliquidacion", "N° " & txtNroFactura.Text)
                objetoRep.SetDataSource(objDs)
                objPrintForm.ShowForm(objetoRep, Me)
            Case 1
                ListaParametros.Add(txtNroFactura.Text)
                ...
                HelpClassST.CrystalReportTextObject(objetoRep_Lote, "lblCompania", _CCMPN_D)
                HelpClassST.CrystalReportTextObject(objetoRep_Lote, "lblDivision", _CDVSN_D)
                HelpClassST.CrystalReportTextObject(objetoRep_Lote, "lblPlanta", _CPLNDV_D)
                HelpClassST.CrystalReportTextObject(objetoRep_Lote, "lblNroPreliquidacion", "N° " & txtNroFactura.Text)
